# feature_encoder/building_blocks/conv_block_standard_stride_down_with_skip.py
...
@register_block('convblock_std_stride')
class ConvBlockStandardStrideDownWithSkip(nn.Module):
    """
    Standard 3x3x3 convolution block with residual skip and stride-based downsampling.
    
    If downsample is True, the first convolution uses stride 2 (followed by one stride-1 conv).
    """
    def __init__(
        self,
        in_channels: int,
        ...
        for i in range(n_convolutions):
            current_stride = stride if (i == 0 and downsample) else 1
            conv = nn.Conv3d(in_channels if i==0 else out_channels, out_channels,
                             kernel_size, stride=current_stride, padding=padding, bias=bias)
            layers = [conv]
