# feature_encoder/building_blocks/conv_block_depthpoint_stride_down_with_skip.py
...
@register_block('convblock_depthpoint_stride')
class ConvBlockDepthPointStrideDownWithSkip(nn.Module):
    """
    Depthwise-pointwise convolution block with residual skip and stride-based downsampling.
    
    Replaces standard convs with a depthwise convolution followed by a pointwise convolution.
    """
    def __init__(
        self,
        in_channels: int,
        ...
        for i in range(n_convolutions):
            ...
            conv = depthpoint_conv(in_channels if i==0 else out_channels, out_channels,
                                   kernel_size, current_stride, padding, bias)
            layers = [conv]
