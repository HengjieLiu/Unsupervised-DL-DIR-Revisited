# feature_encoder/building_blocks/conv_block_standard_maxpool_down_with_skip.py
...
@register_block('convblock_std_maxpool')
class ConvBlockStandardMaxPoolDownWithSkip(nn.Module):
    """
    Standard 3x3x3 convolution block with residual skip and max pooling based downsampling.
    
    If downsample is True, input is first downsampled via max pooling (kernel_size=2, stride=2)
    before applying two stride-1 convolution layers.
    """
    def __init__(
        self,
        in_channels: int,
